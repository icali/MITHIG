...
my $VERSION = sprintf "%d.%03d", q$Revision: 357234 $ =~ /(\d+)/g;
my $verbose;
...
#
   ...
   # \RCS$Revision: 357234 $
   # \RCS$Date: 2016-07-19 21:50:59 +0200 (Tue, 19 Jul 2016) $
   # \RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/utils/branches/dev/general/makeManifest.pl $
   # \RCS$Id: makeManifest.pl 357234 2016-07-19 19:50:59Z alverson $
...
#      &insert_datafield("859",$record,'cds.support@cern.ch');
      ...
      if ($thumbnails) 
      {
          if ($^O eq "MSWin32")
          {
              $ENV{'PATH'} = $ENV{'ProgramFiles'}."\\ImageMagick-6.9.2-Q16;".$ENV{'ProgramFiles'}."\\gs\\gs9.18\\bin;$ENV{'PATH'}";
